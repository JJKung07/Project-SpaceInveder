...
public class Character extends Pane {
    ...
    public Character (int x, int y,int offsetX,int offsetY, KeyCode LEFT,KeyCode RIGHT,KeyCode SPACE,KeyCode B){
        ...
        this.Bshoot = B;
        // Fix the image loading issue
        try {
            InputStream imgStream = Launcher.class.getResourceAsStream("/se233/project2/spaceship.png");
            if (imgStream == null) {
                throw new RuntimeException("spaceship.png resource not found.");
            }
            this.characterImg = new Image(imgStream);
        } catch (Exception e) {
            throw new RuntimeException("Failed to load spaceship image: " + e.getMessage(), e);
        }
